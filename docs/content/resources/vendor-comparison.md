...
A third data point comes from running AI-assisted translation of the same CardDemo corpus to NestJS. That produced a working first draft, but an independent AI review surfaced systematic issues (field name drift, financial arithmetic anti-patterns, business logic divergence in accumulation loops) that required a second pass before the code was correct. The process is comprehension-first, then AI translation, then AI review, then parallel validation. AWS Transform skips the comprehension and review steps, which is why the output requires their proprietary runtime and cannot be maintained without BluAge expertise.
