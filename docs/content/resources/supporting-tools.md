...
Mainframe source control varies by shop. Common systems include Panvalet (Broadcom), CA Endevor (Broadcom), and Librarian (Broadcom). The extraction approach is similar for all of them: bulk export, EBCDIC to ASCII conversion, and a single initial commit to Git, with changes tracked in Git from that point forward. The key differences are in the export tooling provided by each vendor. What is lost in every case is the full version history, change annotations, and member attributes. Common pitfalls regardless of source system include EBCDIC encoding conversion, fixed-width record format handling, and members that contain non-source artifacts.
